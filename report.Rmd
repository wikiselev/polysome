...
```{r, echo=FALSE, cache=TRUE}
head(res.l.le.deseq[order(res.l.le.deseq$padj), c(1,7,8,9)], 25)
```
...
plot_genes(head(res.l.le.deseq[order(res.l.le.deseq$padj),], 25)$ensembl_gene_id)
```


Top 25 most significant genes in LEKU vs L comparison:

```{r, echo=FALSE, cache=TRUE}
head(res.l.leku.deseq[order(res.l.leku.deseq$padj), c(1,7,8,9)], 25)
```

```{r, echo=FALSE, cache=TRUE, fig.width=8, fig.height=5}
plot_genes(head(res.l.leku.deseq[order(res.l.leku.deseq$padj),], 25)$ensembl_gene_id)
```


Top 25 most significant genes in LEKU vs LE comparison:

```{r, echo=FALSE, cache=TRUE}
head(res.le.leku.deseq[order(res.le.leku.deseq$padj), c(1,7,8,9)], 25)
```

```{r, echo=FALSE, cache=TRUE, fig.width=8, fig.height=5}
plot_genes(head(res.le.leku.deseq[order(res.le.leku.deseq$padj),], 25)$ensembl_gene_id)
```


Let us now consider LE vs L comparison and plot top 25 polysome profiles for the shortest:

```{r, echo=FALSE, cache=TRUE}
head(res.l.le.deseq[order(res.l.le.deseq$gene_length), c(1,7,8,9)], 25)
```

```{r, echo=FALSE, cache=TRUE, fig.width=8, fig.height=5}
plot_genes(head(res.l.le.deseq[order(res.l.le.deseq$gene_length),], 25)$ensembl_gene_id)
```
...
# Dependencies between expression and translation activities
...
Let us consider separately these 2 gene sets (genes that change both expression level and polysome fraction; and genes that only change polysome fraction and do not change expression level at all).
...
```{r, echo=FALSE, cache=TRUE, fig.width=8, fig.height=5}
plot_genes(head(t$ensembl_gene_id, 25))
```
...
Genes in which both expression and polysome fraction change have more significant p-values for differences between LE and L conditions. However, the gene legnth distrubutions are approximately the same for both conditions.
